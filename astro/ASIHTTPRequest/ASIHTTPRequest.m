...
- (void)setDelegate:(id)newDelegate
{
    ...
    NSString *delegateDescription = [[newDelegate class] description];
    self.delegateIsa = (int)objc_getClass([delegateDescription UTF8String]);
	[[self cancelledLock] unlock];
}
...
- (void)requestStarted
{
    if (delegate==nil || (int)object_getClass(delegate) != self.delegateIsa) {
        return;
    }
    ...
    if (delegate!=nil) {
        if ((int)object_getClass(delegate) == self.delegateIsa) {
